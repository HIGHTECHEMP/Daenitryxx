...
export async function getDriverContext() {
  ...
  // Keep compatibility with the existing auth context.
  // requireRole() provides the authenticated user at ctx.session.user.
...
export function bindDriverSidebar() {
...
export function setDriverName(ctx) {
  const name =
...
export function bindDriverSignOut(ctx) {
  const buttons = $$("[data-signout], #sign-out");
...
export function bindDriverNotifications(ctx) {
  ...
  const loadNotifications = async () => {
    ...
    $$("[data-read-notification]", list).forEach((button) => {
      button.addEventListener("click", async (event) => {
        event.stopPropagation();
  ...
  // Always start closed. Move the drawer to <body> so it cannot be
  // trapped behind dashboard stacking contexts/transformed parents.
...
notificationCloseButton.addEventListener("click", event => {
  ...
  const openPanel = async () => {
    ...
    panel.hidden = false;
    // Force the browser to acknowledge the closed position before opening.
  ...
  if (markAll) {
    markAll.addEventListener("click", async (event) => {
      ...
      if (error) {
        toast(
          error.message || "Unable to mark notifications.",
          "error"
        );
        return;
      }
...
export async function initDriverDashboard() {
  ...
  bindDriverSidebar();
  setDriverName(ctx);
  bindDriverSignOut(ctx);

  const notifications = bindDriverNotifications(ctx);
