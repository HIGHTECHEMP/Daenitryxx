-- DAENITRYXX production schema for Supabase PostgreSQL.
-- Run in Supabase SQL editor. Review storage bucket/policies and provider configuration before launch.
create extension if not exists pgcrypto;

create type public.user_role as enum ('customer','driver','admin');
create type public.shipment_status as enum ('pending','confirmed','driver_assigned','picked_up','in_transit','out_for_delivery','delivered','cancelled','failed_delivery');
create type public.application_status as enum ('pending','approved','rejected');
create type public.assignment_status as enum ('offered','accepted','rejected','completed','cancelled');
create type public.payment_status as enum ('pending','successful','failed','refunded','cancelled');
...
create table public.driver_applications(
 ...
 license_reference text not null check (char_length(license_reference) between 2 and 120),
 notes text,
...
create table public.drivers(
 ...
 verification_status public.application_status not null default 'approved',
 created_at timestamptz not null default now(),
 updated_at timestamptz not null default now()
);
...
create table public.shipments(
 id uuid primary key default gen_random_uuid(),
 tracking_number text not null unique default ('DX-' || upper(substr(replace(gen_random_uuid()::text,'-',''),1,10))),
 ...
 package_weight_kg numeric(10,2) check (package_weight_kg is null or package_weight_kg>=0),
...
create table public.shipment_documents(
 ...
 document_type text not null check (document_type in ('proof_of_delivery','driver_document','other')),
...
create table public.payments(
 ...
 customer_id uuid not null references public.profiles(id),
 provider text not null check (provider in ('paystack','flutterwave','other')),
...
create index shipments_driver_idx on public.shipments(driver_id,created_at desc);
...
create index history_shipment_idx on public.shipment_status_history(shipment_id,created_at);
create index assignments_driver_idx on public.delivery_assignments(driver_id,status);
...
create trigger applications_updated before update on public.driver_applications for each row execute function public.set_updated_at();
...
begin
 insert into public.profiles(id,full_name,phone) values(new.id,coalesce(new.raw_user_meta_data->>'full_name',split_part(new.email,'@',1)),new.raw_user_meta_data->>'phone')
 on conflict(id) do nothing;
 account_type := coalesce(new.raw_user_meta_data->>'account_type','customer');
 if account_type = 'driver' then
   insert into public.driver_applications(user_id,full_name,phone,vehicle_type,license_reference,notes)
   values(
     new.id,
     coalesce(nullif(trim(new.raw_user_meta_data->>'full_name'),''),split_part(new.email,'@',1)),
     coalesce(nullif(trim(new.raw_user_meta_data->>'phone'),''),''),
     coalesce(nullif(trim(new.raw_user_meta_data->>'vehicle_type'),''),''),
     coalesce(nullif(trim(new.raw_user_meta_data->>'license_reference'),''),''),
...
create or replace function public.submit_driver_application(p_full_name text,p_phone text,p_vehicle_type text,p_license_reference text,p_notes text default null)
returns uuid language plpgsql security definer set search_path=public as $$
declare aid uuid;
begin
 if auth.uid() is null then raise exception 'Authentication required'; end if;
 update public.profiles set full_name=trim(p_full_name),phone=trim(p_phone) where id=auth.uid();
 insert into public.driver_applications(user_id,full_name,phone,vehicle_type,license_reference,notes)
 values(auth.uid(),trim(p_full_name),trim(p_phone),trim(p_vehicle_type),trim(p_license_reference),p_notes) returning id into aid;
 return aid;
end $$;
...
begin
 if auth.uid() is null then raise exception 'Authentication required'; end if;
 if exists(select 1 from public.profiles where id=auth.uid() and role='admin') then raise exception 'Admins cannot create customer shipments'; end if;
...
begin
 ...
 if not exists(select 1 from public.drivers where user_id=p_driver_id and verification_status='approved') then raise exception 'Driver is not approved'; end if;
 insert into public.delivery_assignments(shipment_id,driver_id) values(p_shipment_id,p_driver_id)
 on conflict(shipment_id) do update set driver_id=excluded.driver_id,status='offered',assigned_at=now(),accepted_at=null,rejected_at=null;
 update public.shipments set driver_id=p_driver_id,status='driver_assigned' where id=p_shipment_id;
 insert into public.shipment_status_history(shipment_id,status,changed_by,note) values(p_shipment_id,'driver_assigned',auth.uid(),'Driver assigned');
 insert into public.notifications(user_id,title,body) values(p_driver_id,'New delivery assignment','You have a new delivery assignment.');
 insert into public.audit_logs(actor_id,action,entity_type,entity_id,details) values(auth.uid(),'driver_assigned','shipment',p_shipment_id,jsonb_build_object('driver_id',p_driver_id));
end $$;

create or replace function public.review_driver_application(p_application_id uuid,p_decision public.application_status,p_reason text default null)
returns void language plpgsql security definer set search_path=public as $$
declare applicant uuid; vehicle text;
begin
 if not public.is_admin() then raise exception 'Admin access required'; end if;
 select user_id,vehicle_type into applicant,vehicle from public.driver_applications where id=p_application_id for update;
 if applicant is null then raise exception 'Application not found'; end if;
 update public.driver_applications set status=p_decision,reviewed_by=auth.uid(),reviewed_at=now(),review_reason=p_reason where id=p_application_id;
 if p_decision='approved' then
   update public.profiles set role='driver' where id=applicant;
   insert into public.drivers(user_id,vehicle_type,verification_status) values(applicant,vehicle,'approved')
   on conflict(user_id) do update set vehicle_type=excluded.vehicle_type,verification_status='approved';
   insert into public.notifications(user_id,title,body) values(applicant,'Driver application approved','Your DAENITRYXX driver application has been approved.');
 else
   insert into public.notifications(user_id,title,body) values(applicant,'Driver application update','Your DAENITRYXX driver application was not approved at this time.');
 end if;
 insert into public.audit_logs(actor_id,action,entity_type,entity_id,details) values(auth.uid(),'driver_application_reviewed','driver_application',p_application_id,jsonb_build_object('decision',p_decision));
end $$;
...
begin
 ...
 update public.shipments set status='cancelled',cancelled_at=now() where id=p_shipment_id;
...
returns void language plpgsql security definer set search_path=public as $$
declare sid uuid; assigned uuid;
begin
 select shipment_id,driver_id into sid,assigned from public.delivery_assignments where id=p_assignment_id;
 if assigned<>auth.uid() then raise exception 'Not authorized'; end if;
 ...
 insert into public.notifications(user_id,title,body) select customer_id,'Shipment update','Your shipment status is now '||replace(p_status::text,'_',' ') from public.shipments where id=sid;
end $$;
...
language sql security definer set search_path=public as $$
 ...
 from public.shipments s where s.tracking_number=upper(trim(p_tracking_number)) limit 1;
$$;

-- Authenticated tracking for a signed-in customer/driver/admin. This is separate from
-- public tracking so the dashboard can enforce ownership/assignment explicitly.
...
begin
 if auth.uid() is null then raise exception 'Authentication required'; end if;
 select s.id into sid from public.shipments s
 where s.tracking_number=upper(trim(p_tracking_number))
 and (s.customer_id=auth.uid() or s.driver_id=auth.uid() or public.is_admin())
 limit 1;
...
create policy application_self on public.driver_applications for select using(user_id=auth.uid() or public.is_admin());
create policy application_admin on public.driver_applications for all using(public.is_admin()) with check(public.is_admin());

create policy drivers_self_admin on public.drivers for select using(user_id=auth.uid() or public.is_admin());
create policy drivers_admin_write on public.drivers for all using(public.is_admin()) with check(public.is_admin());

create policy shipments_customer on public.shipments for select using(customer_id=auth.uid() or driver_id=auth.uid() or public.is_admin());
create policy shipments_customer_insert on public.shipments for insert with check(customer_id=auth.uid());
create policy shipments_customer_update on public.shipments for update using(customer_id=auth.uid() or public.is_admin()) with check(customer_id=auth.uid() or public.is_admin());
create policy shipments_admin on public.shipments for all using(public.is_admin()) with check(public.is_admin());

create policy history_visible on public.shipment_status_history for select using(exists(select 1 from public.shipments s where s.id=shipment_id and (s.customer_id=auth.uid() or s.driver_id=auth.uid() or public.is_admin())));
create policy history_admin_insert on public.shipment_status_history for insert with check(public.is_admin() or changed_by=auth.uid());

create policy assignments_driver_admin on public.delivery_assignments for select using(driver_id=auth.uid() or public.is_admin());
create policy assignments_admin_write on public.delivery_assignments for all using(public.is_admin()) with check(public.is_admin());
create policy assignments_driver_update on public.delivery_assignments for update using(driver_id=auth.uid()) with check(driver_id=auth.uid());

create policy docs_owner_driver_admin on public.shipment_documents for select using(uploaded_by=auth.uid() or public.is_admin() or exists(select 1 from public.shipments s where s.id=shipment_id and s.customer_id=auth.uid()));
create policy docs_driver_insert on public.shipment_documents for insert with check(uploaded_by=auth.uid() and public.is_driver());
...
create policy earnings_driver_admin on public.driver_earnings for select using(driver_id=auth.uid() or public.is_admin());
create policy payouts_driver_admin on public.driver_payouts for select using(driver_id=auth.uid() or public.is_admin());
...
create policy support_public_insert on public.support_requests for insert with check(user_id=auth.uid() or user_id is null);
create policy support_owner_admin on public.support_requests for select using(user_id=auth.uid() or public.is_admin());
...
create policy audit_insert_admin_or_self on public.audit_logs for insert with check(public.is_admin() or actor_id=auth.uid());

-- Storage: create a private bucket named delivery-proofs in the Supabase dashboard.
-- Recommended policies:
-- SELECT: authenticated users can read only when they are the uploader, assigned driver, shipment customer, or admin.
-- INSERT: authenticated drivers only, and object name begins with auth.uid() || '/'.
-- UPDATE/DELETE: uploader or admin.


-- =========================================================
-- HARDENED PRODUCTION LAYER
-- =========================================================
